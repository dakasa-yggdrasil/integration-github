...
RUN --mount=type=cache,target=/go/pkg/mod         --mount=type=cache,target=/root/.cache/go-build         go build -o /bin/integration-github .

FROM gcr.io/distroless/base-debian12:nonroot

WORKDIR /app

COPY --from=build /bin/integration-github /app/integration-github

EXPOSE 8080
ENTRYPOINT ["/app/integration-github"]
